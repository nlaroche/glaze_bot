...
- **Edge functions deploy with `--no-verify-jwt`** — Supabase Auth issues ES256 tokens now. The relay's `verify_jwt` only supports HS256. Auth is handled in function code via `getUser()` call to the Auth server.
